...
{
    cbloom_bitmap *map = bitmapFromFile(-1, 0);
    fail_unless(map == NULL);
}
END_TEST
...
{
    cbloom_bitmap *map = bitmapFromFile(500, 4096);
    fail_unless(map == NULL);
}
END_TEST
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/asdf123", 4096, 0, 0);
    fail_unless(map == NULL);
}
END_TEST
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_nofile_create", 4096, 1, 1);
...
START_TEST(make_bitmap_resize)
{
    ...
    fchmod(fh, 0777);
    fail_unless(fh > 0);
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_resize", 4096, 0, 1);
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_flush_bitmap", 8196, 1, 1);
    fail_unless(map != NULL);
    fail_unless(bitmapFlush(map) == 0);
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_close_bitmap", 8196, 1, 1);
    fail_unless(map != NULL);
    fail_unless(bitmapClose(map) == 0);
    fail_unless(map->mmap == NULL);
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_getbit_zero", 4096, 1, 1);
    fail_unless(map != NULL);
    for (int idx = 0; idx < 4096*8 ; idx++) {
        fail_unless(BITMAP_GETBIT(map, idx) == 0);
    }
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_getbit_one", 4096, 1, 1);
    fail_unless(map != NULL);
    memset(map->mmap, 255, 4096);
    for (int idx = 0; idx < 4096*8 ; idx++) {
        fail_unless(BITMAP_GETBIT(map, idx) == 1);
...
{
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_setbit_one", 4096, 1, 1);
    fail_unless(map != NULL);
    for (int idx = 0; idx < 4096*8 ; idx++) {
        BITMAP_SETBIT(map, idx, 1);
    }
...
START_TEST(flush_does_write) {
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_flush_write", 4096, 1, 1);
    ...
    cbloom_bitmap *map2 = bitmapFromFilename("/tmp/mmap_flush_write", 4096, 0, 0);
    fail_unless(map2 != NULL);
    for (int idx = 0; idx < 4096; idx++) {
        fail_unless(map->mmap[idx] == 255);
    }
...
START_TEST(close_does_flush) {
    cbloom_bitmap *map = bitmapFromFilename("/tmp/mmap_close_flush", 4096, 1, 1);
    fchmod(map->fileno, 0777);
    fail_unless(map != NULL);
    for (int idx = 0; idx < 4096*8 ; idx++) {
        BITMAP_SETBIT(map, idx, 1);
    ...
    map = bitmapFromFilename("/tmp/mmap_close_flush", 4096, 0, 0);
    fail_unless(map != NULL);
    for (int idx = 0; idx < 4096; idx++) {
        fail_unless(map->mmap[idx] == 255);
    }
